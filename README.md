...
An extension of [cfp-ai-tracker](https://github.com/csepka/cfp-ai-tracker) for the **Super Bowl**. Event-driven NFL tracker with **Gemini-powered live commentary**: same FSM (PREGAME → LIVE → FINAL), edge-triggered updates, and win-probability model, now using the **Gemini API** for all commentary and postgame recap.
